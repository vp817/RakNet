...
namespace RakNet
{
	...
	private:
		...
		struct UnreliableWithAckReceiptNode
		{
			...
			UnreliableWithAckReceiptNode(DatagramSequenceNumberType _datagramNumber, uint32_t _sendReceiptSerial, RakNet::TimeUS _nextActionTime) : datagramNumber(_datagramNumber), sendReceiptSerial(_sendReceiptSerial), nextActionTime(_nextActionTime)
			{
			}
